...
function checkCoPageTitle(){
    ...
    browser.wait(EC.titleIs('Cielostar cost savings on medical benefits | CBANC'), 5000);
    browser.sleep(2000);
}
function checkTitleOnPage(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitao Introducing title on page
    browser.wait(EC.textToBePresentInElement($('.top_left'),'Introducing'), 5000);
}
function checkForm(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitala forma
    browser.wait(EC.textToBePresentInElement($('#request_form'),'REQUEST A QUOTE'), 5000);
}
 module.exports = {
    clickOnLink,
    checkCoUrl,

...
function checkRaPageTitle(){
    ...
    browser.wait(EC.titleIs('Risk Assessment for Financial Institutions | CBANC'), 7000);
}
function checkTitleOnPage (){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitao title on page
    browser.wait(EC.textToBePresentInElement($('.hero'),'Risk Assessments for Financial Institutions'), 7000);
    browser.sleep(2000);
}    
function checkHelpBox(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitao box for help
    browser.wait(EC.textToBePresentInElement($('.content_footer'),'Want some more information before trying it out?'), 7000);
    browser.sleep(2000);
}    

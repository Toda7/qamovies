function clickOnLink() {
 ...
 function checkOnDemand() {
    ...
    browser.wait(EC.urlContains('https://qa.cbancnetwork.com/education/webinars/on_demand'), 5000);
    browser.sleep(2000);
}
...
function checkOnDemandWebinarsPageTitle(){
    ...
    browser.wait(EC.titleIs('CBANC: On Demand Library'), 5000);
}

function checkTitleOnPage(){
    var EC = protractor.ExpectedConditions;
    //  Provera da li se ucitao page title
    browser.wait(EC.textToBePresentInElement($('.header_meta'), 'On Demand Webinars'), 5000);
}

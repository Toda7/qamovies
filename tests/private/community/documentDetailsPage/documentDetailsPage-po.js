function checkBreadcrumbs() {
    ...
    browser.wait(EC.textToBePresentInElement($('.nav_title'), 'laqasadipapel'), 7000);
}

function checkDocIcon(){
    // Provera da li se ucitao doc icon(pdf)
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.community .document_detail .document_title .file_extension'), 'PDF'), 7000);
}

function checkDocTitle(){
    // Provera da li se ucitao doc title
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.community .document_detail .document_title .constrain h1'), 'laqasadipapel'), 7000);
}
...
function checkUserName(){
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.community .signature .constrain .title .person_name'), 'Memphis 92 FI Foreva'), 7000);
}
...
function checkFunctionLevel(){
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.community .signature .constrain .info .org_function'), 'President - Payments and Electronic Banking'), 7000);
}

function checkOrgLink(){
    // Provera da li se ucitao org link
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.community .signature .constrain .info .org_link'), 'Bank of Memphis'), 7000);
}

function checkOrgSize(){
    // Provera da li se ucitao org size
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.community .signature .constrain .info .org_size'), '($790M | TN)'), 7000);
}
...
function checkTopicURL() {
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.urlContains('https://qa.cbancnetwork.com/community/topics/view/907e0d79-2e57-449e-9864-8e75eb1a59f3'), 7000);
}
...
function checkDirectMessageURL() {
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.urlContains('https://qa.cbancnetwork.com/dm/compose?person_key=18c37233-abbf-428a-b586-2cb97014bafc'), 7000);
}
...
function checkBankURL() {
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.urlContains('https://qa.cbancnetwork.com/community/fis/view/b5d7eb00-76bd-41c3-92ec-0a8f5b7048e9'), 7000);
}
...
function checkDiscussionURL() {
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.urlContains('https://qa.cbancnetwork.com/community/discussions/view/67d84c00-9730-494e-993c-a835b9ea5d31'), 7000);
}

...
function checkTitleInMessage() {
    ...
    browser.wait(EC.textToBePresentInElement($('.documents .background_overlay .modal .modal_content'), 'Thanks for downloading!'), 5000);
}

function checkQuestionInMessage() {
    // Provera da li se ucitalo pitanje u poruci
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.documents .background_overlay .modal .modal_content .question span'), 'Are you interested in occasionally receiving content directly from'), 5000);
}

function checkThankSectionInMessage() {
    // Provera da li se ucitala Thank recenica u poruci
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.textToBePresentInElement($('.documents .background_overlay .modal .modal_content'), 'Thanks for helping keep CBANC free by supporting our vendor partners.'), 5000);
}

...
function checkRaHomeURL() {
    ...
    browser.wait(EC.urlIs('https://qa.cbancnetwork.com/ra/b0fcf98f-a435-4c19-bae0-bb9b15bb94b4/catalog'), 5000);
    browser.sleep(2000);
}
...
function checkTitleInHeader(){
    ...
    browser.wait(EC.textToBePresentInElement($('.page-title'), 'Catalog'), 5000);
}    
...
function checkMenuOnLeftSide(){
    ...
    browser.wait(EC.presenceOf($('.collection.page_menu')), 5000);
}
function checkTemplate(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitao template na strani
    browser.wait(EC.presenceOf($('.card-content')), 5000);
}
...
function checkCreateButton(){
    ...
    browser.wait(EC.textToBePresentInElement($('.card-action'), 'CREATE NEW'), 5000);
}   

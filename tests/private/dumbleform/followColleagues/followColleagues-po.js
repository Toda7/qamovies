...
function checkHelpTitleOnHomePage(){
    ...
    browser.wait(EC.textToBePresentInElement($('.wizard_home'), 'Help Us Help You!'), 7000);
}   
function checkHelpSummaryOnHomePage(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se pojavio summary ispod title on page
    browser.wait(EC.textToBePresentInElement($('.middle'), 'Please take a moment to edit or confirm your profile information.'), 7000);
}  
...
function checkFolloweColleaguesURL() {
    ...
    var EC = protractor.ExpectedConditions;
    browser.wait(EC.urlIs('https://qa.cbancnetwork.com/dumbleform/23479f65-f4ac-4295-bb22-c446519fec15/follow_colleagues'), 7000);
    browser.sleep(2000);
}
function checkTitleOnPage(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitao title on page
    browser.wait(EC.textToBePresentInElement($('.dumbleform'), 'Follow Colleagues'), 7000);
}  
function checkDescriptionOnPage(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se ucitao description ispod title on page
    browser.wait(EC.textToBePresentInElement($('.dumbleform'), 'Following the right people can increase the benefit you get out of the CBANC Network.'), 7000);
}  
...
function checkInviteColleaguesTitleOnHomePage(){
    ...
    browser.wait(EC.textToBePresentInElement($('.dumbleform'), 'Invite Colleagues'), 7000);
}   
function checkInviteColleaguesDescriptionOnHomePage(){
    var EC = protractor.ExpectedConditions;
    // Provera da li se pojavio Invite Colleagues description on page
    browser.wait(EC.textToBePresentInElement($('.dumbleform'), 'A more powerful, helpful network starts with people like you. Send a friendly invitation to your colleagues.'), 7000);
}   
function checkInvitesThatWellSend(){
    var EC = protractor.ExpectedConditions;
    // Provera da li postoji vec uneti email u invite formu, trebalo bi da bude 0 
    browser.wait(EC.textToBePresentInElement($('.main.dumbleform_invite_colleagues h4'), '0'), 7000);
}   
...
function checkFollowVendorsTitleOnHomePage(){
    ...
    browser.wait(EC.textToBePresentInElement($('.dumbleform'), 'Follow Vendors'), 7000);
}  
